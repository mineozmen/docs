...
* [Server Sent Events](server-sent-events.md) explains how `/api/stream/...` turns repeated saga calls into an SSE feed using `list`, `continue`, `wait`, and `next`.
* [Rate Limiting](rate-limiting.md) covers built-in user and IP based throttling at gateway and channel level.
...
* Go to [Rate Limiting](rate-limiting.md) or [Dynamic TLS & mTLS](dynamic-tls-and-mtls.md) when hardening production traffic.
