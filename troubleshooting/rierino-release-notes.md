...
UI builder now allows fully custom-coded screens, which can be generated by our AI agents, giving full autonomy to business users for generating their own screens with custom layouts, components and functionality.

### Claude Agents as Marketplace Plugin

Rierino development agents, skills and MCP server is now available as a Claude marketplace plugin, simplifying its installation on Claude Code and Claude Desktop for AI assisted development of sagas, queries, UIs and more.

### AI Monitoring Dashboard

Tracking token & tools usage as well as user feedback on AI agents is simplified with built-in AI monitoring dashboard, visualizing usage history and preferences of all GenAI models.

### Switchable Multi-Lister

New lister switching feature allows users to pick their preferred way to list records (tables, maps, boards, custom listers, etc.) as per their use case, allowing different users to view and use same UIs in different ways.

### Granular API Gateway Concurrency Control

Channel level concurrency control allows system administrators to limit concurrent calls, for optimizing thread and resource distribution across channels based on their importance and expected load.

### Simplified Runner Editor

A new specialized editor is added for managing runners, making it easier to oversee and edit runners with high number of elements.
